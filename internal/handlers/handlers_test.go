...
var theTest = []struct {
	...
	/*
		{"post-search-avail", "/search-availability", "POST", []postData{
			{key: "start", value: "2023-12-09"},
			{key: "end", value: "2023-12-24"},
		}, http.StatusOK},
...
func TestRepository_Reservation(t *testing.T) {
	...
	if rr.Code != http.StatusOK {
		t.Errorf("Reservation handler returner wrong response code: got %d, wanted %d", rr.Code, http.StatusOK)
	}

	// test case where reservation is not in session (reset everzthing)
	req, _ = http.NewRequest("GET", "/make-reservation", nil)
	ctx = getCtx(req)
	req = req.WithContext(ctx)
	rr = httptest.NewRecorder()
	...
	if rr.Code != http.StatusTemporaryRedirect {
		t.Errorf("Reservation handler returned wromg response code: got %d, wanted %d", rr.Code, http.StatusTemporaryRedirect)
	}
