...
func (m *Repository) Reservation(w http.ResponseWriter, r *http.Request) {
	...
	room, err := m.DB.GetRoonByID(res.RoomID)
	if err != nil {
...
func (m *Repository) ReservationSummary(w http.ResponseWriter, r *http.Request) {
	reservation, ok := m.App.Session.Get(r.Context(), "reservation").(models.Reservation)
	if !ok {
		m.App.ErrorLog.Println("Cannot get item fromSession")
	...
	sd := reservation.StartDate.Format("2020-01-02")
	ed := reservation.EndDate.Format("2020-01-02")
	stringMap := make(map[string]string)
	stringMap["start_date"] = sd
	stringMap["end_date"] = ed
...
func (m *Repository) BookRoom(w http.ResponseWriter, r *http.Request) {
	...
	room, err := m.DB.GetRoonByID(roomID)
	if err != nil {
		helpers.ServerError(w, err)
		return
	}
